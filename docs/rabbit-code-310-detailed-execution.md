...
| 当前状态 | RC-036 已提交调研汇总、功能对比、ADR/许可证/不复用索引和跨模块评审门禁；M0 签收待确认，准备执行 RC-037 |
| 当前阻塞 | RC-036 的 M0 评审、重复 ADR-0003 编号修正和后续架构 Issue 决策映射待确认；不阻塞 RC-037 |
| 最近一次完整验证 | 2026-07-17：RC-036 汇总校验、RC-035 授权/发布阻断、RC-034 名称渠道、Ruff、第三方登记、source-map denylist、专有内容政策与 RC 追踪检查通过；官方 Codex manual 当前 HTTP 200 已由 RC-017 记录 |
| 最近更新时间 | 2026-07-17 12:17:01 +08:00 |
| 更新人/Agent | Codex |
...
| RC-036 | 2026-07-17 12:17:01 +08:00 | 待本项记录提交后固定 | SUBMITTED WITH PENDING CONFIRMATION：六组来源、ADR/许可证/不复用索引和 M0/架构 Issue 门禁已提交；校验器与 Ruff 通过 | `docs/evidence/RC-036/README.md` | M0 逐份签收、重复 ADR-0003 修正和后续架构 Issue 决策映射待确认 |
...
- [ ] **RC-037** 定义目标用户：个人开发者、无 API 用户、多 Provider 用户、开源贡献者和团队开发者。
- [ ] **RC-038** 定义首发平台：Windows、macOS、Linux，并分别列出 CPU 架构、终端、Shell、GPU 和安装包支持矩阵。
- [ ] **RC-039** 定义核心场景：打开仓库、发起编码任务、阅读代码、规划、编辑、运行命令、测试、审查 diff、恢复会话、切换模型、优化提示词和本地离线对话。
...
- [ ] **RC-044** 制定版本策略、兼容策略、废弃策略、数据库迁移策略和配置迁移策略。
- [ ] **RC-045** 在技术预研后制定实际开发时间表、人员/模块负责人、依赖关系、风险缓冲和里程碑日期。
