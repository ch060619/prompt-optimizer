...
def test_config_fields_are_typed_scoped_and_secrets_are_not_displayed(tmp_path: Path) -> None:
    ...
    store.put(reference, "runtime-secret")
    user_path.write_text(
        json.dumps({"api_key": f"keychain:{reference}", "timeout_seconds": 12.5}),
        encoding="utf-8",
    )
    service = ConfigService(user_path=user_path, secret_store=store)
    ...
    transient = service.resolve(session={"api_key": "temporary-secret"})
    assert transient.values()["api_key"] == "temporary-secret"
    assert "temporary-secret" not in json.dumps(transient.display(), ensure_ascii=False)
...
def test_config_cli_shows_sources_without_secret_values(tmp_path: Path, monkeypatch) -> None:
    user_path = tmp_path / "user.json"
    user_path.write_text(json.dumps({"provider": "openai"}), encoding="utf-8")
    monkeypatch.setenv("RABBIT_CODE_API_KEY", "secret-from-environment")
    workspace_root = tmp_path / "workspace"
    workspace_path = workspace_root / ".rabbit-code" / "config.json"
    workspace_path.parent.mkdir(parents=True)
    ...
    assert "secret-from-environment" not in result.stdout
